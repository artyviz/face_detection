...
IMAGE_DIR = Path("C:/Users/Farhan/Desktop/face detection/images") # your own image folder path
IPCAM_URL = 'http://192.168.231.162/' #custom ip of ur arduino
...
def capture_thread(url, q, stop_evt):
    while not stop_evt.is_set():
        try:
            ...
            data = r.read()
            imnp = np.frombuffer(data, dtype=np.uint8)
            frame = cv2.imdecode(imnp, cv2.IMREAD_COLOR)
...
t = threading.Thread(target=capture_thread, args=(IPCAM_URL, frame_queue, stop_event), daemon=True)
t.start()
